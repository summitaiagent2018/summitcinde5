...
app.intent('Default Welcome Intent', conv => 
{
    ...
    else
    {
        ...
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.defaultWelcomeMsgVoiceDevice.mainMsg'),
            text: config.get('responseMessages.defaultWelcomeMsgVoiceDevice.mainMsg'),        
        }));   
...
app.intent('Get Signin', (conv, params, signin) => {
    ...
    var displayText = '';
    var textToSpeech = '';
    var chatBotID = config.get('App.Auth.chatBotID');

    if (signin.status !== 'OK') 
    {
        return conv.ask(config.get('responseMessages.onSignInFailure'));
    }
            ...
            if(userDetailsJSON != '')
            {
                ...
                GetSummitUserDetails(authUserEmailId,chatBotID,function(userExistsJSON)
                {
                    ...
                    if (userExistsJSON.Remarks == "USER_EXISTS") {
                        ...
                        displayText = util.format(config.get('responseMessages.onSignInSuccess'), summitUserName);
                        textToSpeech = util.format(config.get('responseMessages.onSignInSuccess'), summitUserName);
                        ...
                        conv.ask(new SimpleResponse({
                            speech: config.get('responseMessages.defaultWelcomeMsgPhoneSurface.mainMsg'),
                            text: config.get('responseMessages.defaultWelcomeMsgPhoneSurface.mainMsg'),
                        }), new Suggestions([config.get('responseMessages.defaultWelcomeMsgPhoneSurface.suggestions.sug1'), config.get('responseMessages.defaultWelcomeMsgPhoneSurface.suggestions.sug2'), config.get('responseMessages.defaultWelcomeMsgPhoneSurface.suggestions.sug3')]));
                        resolve();

                    }
                    else
                    {
                        displayText = config.get('responseMessages.inValidUser');
                        textToSpeech = config.get('responseMessages.inValidUser');
                        conv.ask(new SimpleResponse({
                            speech: textToSpeech,
                            text: textToSpeech,
                        }));
...
app.intent('Log_Incident_Utterances',function (conv) 
{
        ...
        return new Promise(function(resolve,reject)
        {
            GetKBArticles(Symptom,function(KBArticlesArr)
            {
                ...
                {    
                    ...
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.questionToDisplayKB'),
                        text: config.get('responseMessages.questionToDisplayKB'),
                ...
                else
                {
                    ...
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.questionForLoggingIncident'),
                        text: config.get('responseMessages.questionForLoggingIncident'),
                    }),new Suggestions(['Yes', 'No']));   
                    resolve();
                }
    ...
    else
    {
        conv.data.symptom = "";
            ...
            conv.ask(new SimpleResponse({
                speech: config.get('responseMessages.defaultFollowUpMsg'),
                text: config.get('responseMessages.defaultFollowUpMsg'),
            }),new Suggestions(['Yes', 'No']));
    }
        ...
        for (var i = 0; i < conv.data.KBArticlesArray.length; i++)
         {
            ...
            Items[conv.data.KBArticlesArray[i].KB_ID].description = conv.data.KBArticlesArray[i].KB_Details;   
                ...
                textToSpeech = util.format(config.get('responseMessages.onSuccessKBConfirmation'), conv.data.symptom);
                displayText = util.format(config.get('responseMessages.onSuccessKBConfirmation'), conv.data.symptom);
...
app.intent('KBs_Useful_Confirmation',function(conv)
{
    ...
    if (confirmation.toLowerCase() == 'helpful')       
    {
        ...
        conv.ask(config.get('responseMessages.onKBArticlesHelpful'));       
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.defaultFollowUpMsg'),
            text: config.get('responseMessages.defaultFollowUpMsg'),
        }),new Suggestions(['Yes','No']));  
    ...
    {
        conv.contexts.set('log_incident_confirmation',1);
        conv.ask(config.get('responseMessages.onKBArticlesNotHelpful'));
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.questionForLoggingIncident'),
            text: config.get('responseMessages.questionForLoggingIncident'),
        }),new Suggestions(['Yes','No']));  
...
app.intent('Log_Incident_confirmation', function(conv, params, confirmation) 
{
    ...
    {
        conv.ask(config.get('responseMessages.getDecriptionMsg'));
    } 
    else 
    {
        conv.data.symptom = "";
        conv.contexts.set('end_conversation',1);
        conv.ask(config.get('responseMessages.OnNoToIncidentLogging'));
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.defaultFollowUpMsg'),
            text: config.get('responseMessages.defaultFollowUpMsg'),
        }),new Suggestions(['Yes', 'No']));
...
app.intent('Get_Description',function(conv)
{
    ...
    if(description !== "" && conv.data.symptom !== "" && conv.data.incidentConfirmator.toLowerCase() === 'yes')
    {
            ...
            {
                displayText = util.format(config.get('responseMessages.OnIncidentLoggedSuccessfully'), json.TicketNo);
                
                numberToDigit(json.TicketNo, function (ticknum)
                {
                    console.log("Corrected Ticket Number",ticknum);
                    textToSpeech = util.format(config.get('responseMessages.OnIncidentLoggedSuccessfully'), ticknum);
                })
            })
    ...
    else
    {
        console.log("else");
        conv.contexts.set('end_conversation',1);
        textToSpeech = config.get('responseMessages.OnIncidentNotLogged');
        displayText = config.get('responseMessages.OnIncidentNotLogged');
    }
    conv.data.symptom = "";
    return asyncTask()
...
app.intent('Fetch_Category',function (conv) 
{
            ...
            {   
                ...
                if (ServiceCatalogDetailsArr.length === 1)
                {
                        ...
                        GetSRCatalogDetails(userID, categoryID, isLeafNode, function (CatalogDetailsArray)
                        {
                            if (isLeafNode == 1)
                            {
                                if (CatalogDetailsArray.length == 1)
                                {
                                    ...
                                    conv.ask(new SimpleResponse({
                                        speech: util.format(config.get('responseMessages.questionToCreateSR'), catalogName),
                                        text: util.format(config.get('responseMessages.questionToCreateSR'), catalogName),
                                    }), new Suggestions(['Yes', 'No']));
                                    resolve();
                                }
                                ...
                                if (CatalogDetailsArray.length > 1)
                                {
                                    ...
                                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                                    {
                                        textToSpeech = config.get('responseMessages.toDisplayCatalogOptions.phoneSurface');
                                    ...
                                    if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                                    {
                                        textToSpeech = util.format(config.get('responseMessages.toDisplayCatalogOptions.voiceDevice'), listValuesString);
                                        conv.contexts.set('fetch_catalog', 1);
                                        conv.ask(textToSpeech);
                                        resolve();
                                    }
                    ...
                    else
                    {
                        ...
                        conv.contexts.set('Log_SR_Confirmation', 1);
                        conv.ask(new SimpleResponse({
                            speech: util.format(config.get('responseMessages.questionToCreateSR'), categoryName),
                            text: util.format(config.get('responseMessages.questionToCreateSR'), categoryName),
                        }), new Suggestions(['Yes', 'No']));
                        resolve();
                    }
                ...
                else
                {
                    ...
                    textToSpeech = config.get('responseMessages.entityNotFound');
    ...
    {
        conv.ask(config.get('responseMessages.cantUnderstandUserInput'));
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.defaultFollowUpMsg'),
            text: config.get('responseMessages.defaultFollowUpMsg'),
        }),new Suggestions(['Yes', 'No']));   
        ...
        return new Promise(function (resolve, reject)
        {
            ...
            GetSRCatalogDetails(userID, categoryID, isLeafNode, function (CatalogDetailsArray)
            {
                if (CatalogDetailsArray.length == 1)
                {
                    ...
                    conv.contexts.set('Log_SR_Confirmation', 1);
                    conv.ask(new SimpleResponse({
                        speech: util.format(config.get('responseMessages.questionToCreateSR'), description),
                        text: util.format(config.get('responseMessages.questionToCreateSR'), description),
                    }), new Suggestions(['Yes', 'No']));
                    resolve();
                ...
                else
                {
                    ...
                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = config.get('responseMessages.toDisplayCatalogOptionsSubMsg.phoneSurface');
                        conv.contexts.set('fetch_catalog', 1);
                        conv.ask(textToSpeech);
                        //Create a list
                        conv.ask(new List({
                    ...
                    if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.toDisplayCatalogOptionsSubMsg.voiceDevice'), listValuesString);
                        conv.contexts.set('fetch_catalog', 1);
                        conv.ask(textToSpeech);
                        resolve();
                    }
...
app.intent('SR_Confirmation',function(conv,params, confirmation)
{
        ...
        return new Promise( function( resolve, reject )
        {
            GetSelectedCatalogDetailsListCall(conv.data.catalogID, function (SelectedCatalogDetailsListArr)
            {
                ...
                {               
                    ...
                    var startStr = config.get('responseMessages.msgToFetchCustomAttriutes.mainMsg');
                    ...
                    if (conv.data.questions.length > 0)
                    {
                        if (conv.data.questions[0].attrType === 'DropdownList')
                        {
                            ...
                            textToSpeech = startStr + util.format(config.get('responseMessages.msgToFetchCustomAttriutes.dropDownMsg'), conv.data.questions[0].dropdownlist.length, conv.data.questions[0].attrName, listValuesString);
                            ...
                            {    
                                ...
                                //Create a list
                                conv.ask(new List({
                                    title: util.format(config.get('responseMessages.msgToFetchCustomAttriutes.itemListMsg'), conv.data.questions[0].attrName),
                                    items: Items
                                }));
                                resolve();
                            }
                        ...
                        {
                            textToSpeech = startStr + util.format(config.get('responseMessages.msgToFetchCustomAttriutes.dateMsg'), conv.data.questions[0].attrName);
                            console.log("Speech: " + textToSpeech);
                            conv.contexts.set('User_Answer',1);
                        ...
                        {
                            textToSpeech = startStr + util.format(config.get('responseMessages.msgToFetchCustomAttriutes.textBoxMsg'), conv.data.questions[0].attrName);
                            console.log("Speech: " + textToSpeech);
                            conv.contexts.set('User_Answer',1);  
                            conv.ask(textToSpeech);
                            resolve();
                        ...
                        else
                        {
                            conv.contexts.set('User_Answer',1);
                            conv.ask(new SimpleResponse({
                                speech: config.get('responseMessages.msgToFetchCustomAttriutes.formulaMsg'),
                                text: config.get('responseMessages.msgToFetchCustomAttriutes.formulaMsg'),
                            }),new Suggestions('OK'));
                            resolve();
                        }
    ...
    else 
    {
        conv.data.category = "";
        conv.contexts.set('end_conversation',1);
        textToSpeech = config.get('responseMessages.requestNotRequired');
        conv.ask(textToSpeech);
        conv.ask(new SimpleResponse({
            speech: config.get('responseMessages.defaultFollowUpMsg'),
            text: config.get('responseMessages.defaultFollowUpMsg'),
...
app.intent('AskQuestions_ReadAnswers_CreateSR',function (conv, params, option) 
{
    ...
    if(userAnswer != "")
    {
        ...
        if (conv.data.questions[0].attrType === 'DropdownList')
        {
            ...
            else
            {
                ...
                {
                    conv.contexts.set('User_Answer',1);
                    conv.ask(config.get('responseMessages.OnInvalidOptionSelection'));
                }
                else
                {
                    ...
                    conv.contexts.set('end_conversation',1);
                    conv.ask(config.get('responseMessages.OnThirdTimeInvalidOptionSelected'));
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.defaultFollowUpMsg'),
                        text: config.get('responseMessages.defaultFollowUpMsg'),
                    }),new Suggestions(['Yes', 'No']));
        ...
        if (conv.data.questions.length === 0 && isAnswerAccepted === 1)
        {
            LogServiceRequestCall(userID,userName,categoryName, categoryID, catalogName, catalogID, conv.data.subjsonArr, function (ticknum)
            {
                ...
                if (ticknum == '')
                {
                    textToSpeech = config.get('responseMessages.OnFailureSRRequest');
                    displayText = config.get('responseMessages.OnFailureSRRequest');
                    conv.contexts.set('end_conversation',1);
                }
                else
                {
                    displayText = util.format(config.get('responseMessages.OnSuccessSRRequest'), ticknum);
                    numberToDigit(ticknum, function (ticknum)
                    {
                        ticknum = "S R " + ticknum;
                        textToSpeech = util.format(config.get('responseMessages.OnSuccessSRRequest'), ticknum);
                        conv.contexts.set('end_conversation',1);
                    })
                }
    ...
    if (conv.data.questions.length > 0 && isAnswerAccepted !== 0)
    {
            if (conv.data.questions[0].attrType === 'DropdownList')
            {
                ...
                textToSpeech = util.format(config.get('responseMessages.msgToFetchCustomAttriutes.dropDownMsg'), conv.data.questions[0].dropdownlist.length, conv.data.questions[0].attrName, listValuesString);
                console.log("Items List====>",Items);
            ...
            {
                textToSpeech = util.format(config.get('responseMessages.msgToFetchCustomAttriutes.dateMsg'), conv.data.questions[0].attrName);
                console.log("Speech: " + textToSpeech);
                conv.contexts.set('User_Answer',1);
                //const contexts = conv.contexts;
                //console.log('Contexts Set====>',contexts);
            ...
            {
                textToSpeech = util.format(config.get('responseMessages.msgToFetchCustomAttriutes.textBoxMsg'), conv.data.questions[0].attrName);
                console.log("Speech: " + textToSpeech);
                conv.contexts.set('User_Answer',1);
                conv.ask(textToSpeech);   
            } 
        ...
        GetTicketDetails(userID,noOfTickets,ticketNumber,function(ticketDetailsArray)
        {
            ...
            if(incidentNumber != "")
            {
                if(incidentStatus.toLowerCase() == "new" || incidentStatus.toLowerCase() == "assigned" || incidentStatus.toLowerCase() == "in-progress" || incidentStatus.toLowerCase() == "pending")
                {
                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.openIncidentStatus.phoneSurface'), incidentStatus);
                    ...
                    {
                        conv.contexts.set('update_open_incident', 1);
                        textToSpeech = util.format(config.get('responseMessages.openIncidentStatus.voiceDevice'), incidentStatus, incidentSymptom, incidentNumber, incidentCreatedDateTime);                      
                        conv.ask(textToSpeech);
                        console.log("textToSpeech", textToSpeech);
                        resolve();
                    }
                ...
                if(incidentStatus.toLowerCase() == "resolved" || incidentStatus.toLowerCase() == "closed" || incidentStatus.toLowerCase() == "cancelled")                   
                {
                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.closedIncidentStatus.phoneSurface'), incidentStatus);
                        conv.ask(textToSpeech);
                        // Create a basic card
                        conv.ask(new BasicCard({
                            text: 'Symptom: ' + incidentSymptom,
                    ...
                    if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.closedIncidentStatus.voiceDevice'), incidentStatus, incidentSymptom, incidentNumber, incidentCreatedDateTime);  
                        conv.ask(textToSpeech);
                        console.log("textToSpeech", textToSpeech);
                        resolve();
                    }
            ...
            if(srNumber != "")
            {
                if(srStatus.toLowerCase() == "new" || srStatus.toLowerCase() == "assigned" || srStatus.toLowerCase() == "in-progress" || srStatus.toLowerCase() == "pending")
                {
                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.openSRStatus.phoneSurface'), srStatus);
                    ...
                    {
                        conv.contexts.set('update_open_SR', 1);
                        textToSpeech = util.format(config.get('responseMessages.openSRStatus.voiceDevice'), srStatus, srDescription, srNumber, srCreateDateTime);
                ...
                if(srStatus.toLowerCase() == "resolved" || srStatus.toLowerCase() == "closed" || srStatus.toLowerCase() == "cancelled")                   
                {
                    if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.closedSRStatus.phoneSurface'), srStatus);
                        conv.ask(textToSpeech);
                        // Create a basic card
                        conv.ask(new BasicCard({
                            text: 'Description: ' + srDescription,
                    ...
                    if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                    {
                        textToSpeech = util.format(config.get('responseMessages.closedSRStatus.voiceDevice'), srStatus, srDescription, srNumber, srCreateDateTime);
                        conv.ask(textToSpeech);
                        resolve();
                    }
            ...
            {
                textToSpeech = util.format(config.get('responseMessages.Incident/SRNotFound'), ticketNumber);
            ...
            {
                textToSpeech = config.get('responseMessages.SameIncidentAndSRNumber');
                conv.ask(textToSpeech);  
                console.log("textToSpeech",textToSpeech);
                resolve();           
            }
...
app.intent('Update_Open_Incidents',function(conv)
{
    ...
    else if(resolvedQuery.toLowerCase() == 'remind the analyst')
    {
        ...
        return new Promise( function( resolve, reject )
        {
            UpdateIncident(conv.data.IncidentNumber,conv.data.userID,conv.data.IncidentStatus,remainderComment,function(OutputJSON)
            {
                ...
                {
                    textToSpeech = config.get('responseMessages.incidentUpdateMsgs.remind');
                    conv.contexts.set('end_conversation',1);
    ...
    {
        textToSpeech = config.get('responseMessages.incidentUpdateMsgs.escalate');
        conv.contexts.set('open_incident_confirmation',1);
    ...
    {
        textToSpeech = config.get('responseMessages.incidentUpdateMsgs.cancel');
        conv.contexts.set('open_incident_confirmation',1);
    ...
    {
        textToSpeech = config.get('responseMessages.incidentUpdateMsgs.noneOfTheAbove');
        conv.contexts.set('update_open_incident',1);
        conv.ask(new SimpleResponse({
            speech: textToSpeech,
            text: textToSpeech,
...
app.intent('Open_Incident_Confirmation',function(conv)
{
    ...
    if(conv.data.incidentUpdateType == 'update')
    {
        return new Promise( function( resolve, reject )
        {
            UpdateIncident(conv.data.IncidentNumber,conv.data.userID,conv.data.IncidentStatus,resolvedQuery,function(OutputJSON)
            {
                if(OutputJSON.Message == 'Ticket details are updated successfully')
                {
                    textToSpeech = config.get('responseMessages.onSuccessIncidentUpdate.update');
                    conv.contexts.set('end_conversation',1);
                    conv.ask(textToSpeech);
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.defaultFollowUpMsg'),
        ...
        if(resolvedQuery.toLowerCase() == 'yes')
        {
            ...
            return new Promise( function( resolve, reject )
            {
                UpdateIncident(conv.data.IncidentNumber,conv.data.userID,conv.data.IncidentStatus,escalationComment,function(OutputJSON)
                {
                    if(OutputJSON.Message == 'Ticket details are updated successfully')
                    {
                        textToSpeech = config.get('responseMessages.onSuccessIncidentUpdate.escalate');
                        conv.contexts.set('end_conversation',1);
                        conv.ask(textToSpeech);
                        conv.ask(new SimpleResponse({
                            speech: config.get('responseMessages.defaultFollowUpMsg'),
    ...
    if(conv.data.incidentUpdateType == 'cancel')
    {
        if(resolvedQuery.toLowerCase() == 'yes')
        {
            textToSpeech = config.get('responseMessages.tofetchIncidentCancelReason');
    ...
    if(conv.data.incidentUpdateType.toLowerCase() == 'cancel' && conv.data.cancelIncidentConfirmation == 'yes')
    {
        ...
        return new Promise( function( resolve, reject )
        {
            CancelIncident(conv.data.IncidentNumber,conv.data.userID,resolvedQuery,function(OutputJSON)
            {
                if(OutputJSON.Message == 'Ticket details are updated successfully')
                {
                    textToSpeech = config.get('responseMessages.onSuccessfulIncidentCancelation');
                    conv.contexts.set('end_conversation',1);
                    conv.ask(textToSpeech);
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.defaultFollowUpMsg'),
...
app.intent('Update_Open_SRs',function(conv)
{
        ...
        return new Promise( function( resolve, reject )
        {
            UpdateServiceRequest(conv.data.SRNumber,conv.data.userID,remainderComment,function(OutputJSON)
            {
                if(OutputJSON.Message == 'Successfully saved the service request.')
                {
                    ...
                    textToSpeech = config.get('responseMessages.SRUpdateMsgs.remind');
                    conv.contexts.set('end_conversation',1);
                    conv.data.incidentUpdateType = 'remind';
                    conv.ask(textToSpeech);
                    conv.ask(new SimpleResponse({
    ...
    {
        textToSpeech = config.get('responseMessages.SRUpdateMsgs.escalate');
        conv.contexts.set('open_SR_confirmation',1);
        conv.data.incidentUpdateType = 'escalate';
        conv.ask(new SimpleResponse({
            speech: textToSpeech,
    ...
    {
        textToSpeech = config.get('responseMessages.SRUpdateMsgs.noneOfTheAbove');;
        conv.contexts.set('update_open_SR',1);
        conv.ask(new SimpleResponse({
            speech: textToSpeech,
            text: textToSpeech,
    ...
    if(conv.data.incidentUpdateType == 'update')
    {
        return new Promise( function( resolve, reject )
        {
            UpdateServiceRequest(conv.data.SRNumber,conv.data.userID,resolvedQuery,function(OutputJSON)
            {
                ...
                {
                    console.log("inside if block");
                    textToSpeech = config.get('responseMessages.onSuccessSRUpate.update');
                    conv.contexts.set('end_conversation',1);
                    conv.ask(textToSpeech);
                    conv.ask(new SimpleResponse({
                        speech: config.get('responseMessages.defaultFollowUpMsg'),
        ...
        if(resolvedQuery.toLowerCase() == 'yes')
        {
            ...
            return new Promise( function( resolve, reject )
            {
                UpdateServiceRequest(conv.data.SRNumber,conv.data.userID,escalationComment,function(OutputJSON)
                {
                    ...
                    {
                        console.log("inside if block");
                        textToSpeech = config.get('responseMessages.onSuccessSRUpate.escalate');
                        conv.contexts.set('end_conversation',1);
                        conv.ask(textToSpeech);
                        conv.ask(new SimpleResponse({
                            speech: config.get('responseMessages.defaultFollowUpMsg'),
...
app.intent('Know_Status_Of_LastIncident',function(conv)
{
    ...
    return new Promise(function(resolve,reject)
    {
        GetIncidentDetails(userID,noOfTickets,function(IncidentDetailsArr)
        {
            ...
            {
                textToSpeech = config.get('responseMessages.noLastIncidentFound');
                conv.contexts.set('identify_SR',1);
                conv.contexts.set('identify_incident',1);
                conv.contexts.set('identify_status',1);
                conv.contexts.set('Last_Incident_Status',1);
    ...
    return new Promise(function(resolve,reject)
    {
        ...
        {        
            ...
            else
            {
                ...
                textToSpeech = config.get('responseMessages.noLastSRFound');
                conv.contexts.set('identify_SR',1);
                conv.contexts.set('identify_incident',1);
                conv.contexts.set('identify_status',1);
                conv.contexts.set('Last_Incident_Status',1);
    ...
    return new Promise(function(resolve,reject)
    {
        GetIncidentDetails(userID,noOfTickets,function(IncidentDetailsArr)
        {
            ...
            {            
                ...
                if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.statusOfMoreThanOneOpenIncidents.phoneSurface'), slicedIncidentDetailsArr.length);                   
                ...
                if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.statusOfMoreThanOneOpenIncidents.voiceDevice'), slicedIncidentDetailsArr.length, listValuesString);                   
                    conv.contexts.set('get_incident_details', 1);
                    conv.ask(textToSpeech);
                    resolve();
                }
            ...
            else if (slicedIncidentDetailsArr.length == 1)
            {
                ...
                if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.openIncidentStatus.phoneSurface'), slicedIncidentDetailsArr[0].Status);;
                    conv.contexts.set('update_open_incident', 1);
                    conv.ask(textToSpeech);
                    // Create a basic card
                    conv.ask(new BasicCard({
                ...
                {
                    conv.contexts.set('update_open_incident', 1);
                    textToSpeech = util.format(config.get('responseMessages.openIncidentStatus.voiceDevice'), slicedIncidentDetailsArr[0].Status, slicedIncidentDetailsArr[0].Symptom, slicedIncidentDetailsArr[0].Incident_ID, slicedIncidentDetailsArr[0].Logged_Time);                   
                    conv.ask(textToSpeech);
                    console.log("textToSpeech", textToSpeech);
                    resolve();
                }
            ...
            else
            {
                ...
                conv.contexts.set('Last_week_incident',1); 
                conv.contexts.set('Last_week_SR',1); 
                conv.ask(config.get('responseMessages.noLastIncidentFound')); 
                resolve();
            }
        })
    })
    ...
    return new Promise(function(resolve,reject)
    {
        GetSRDetails(userID,noOfTickets,function(SRDetailsArr)
        {
            ...
            {            
                ...
                if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.statusOfMoreThanOneOpenSRs.phoneSurface'), slicedSRDetailsArr.length);
                ...
                if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.statusOfMoreThanOneOpenIncidents.voiceDevice'), slicedSRDetailsArr.length, listValuesString);                  
                    conv.contexts.set('get_SR_details', 1);
                    conv.ask(textToSpeech);
                    resolve();
                }
            ...
            else if (slicedSRDetailsArr.length == 1)
            {
                ...
                if (conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT'))
                {
                    textToSpeech = util.format(config.get('responseMessages.openSRStatus.phoneSurface'), slicedSRDetailsArr[0].Status);
                    conv.contexts.set('update_open_SR', 1);
                    conv.ask(textToSpeech);
                    // Create a basic card
                    conv.ask(new BasicCard({
                ...
                {
                    conv.contexts.set('update_open_SR', 1);
                    textToSpeech = util.format(config.get('responseMessages.openSRStatus.voiceDevice'), slicedSRDetailsArr[0].Status, slicedSRDetailsArr[0].Description, slicedSRDetailsArr[0].Ticket_No, slicedSRDetailsArr[0].Reg_Time);                    
                    conv.ask(textToSpeech);
                    console.log("textToSpeech", textToSpeech);
                    resolve();
                }
            ...
            else
            {
                ...
                conv.contexts.set('Last_week_incident',1);
                conv.contexts.set('Last_week_SR',1); 
                conv.ask(config.get('responseMessages.noLastSRFound')); 
                resolve();
            }
        })
    })
...
function GetKBArticles(incidentFor, callback) {
        ...
        objCommonParameters:
         {
             _ProxyDetails:
              {
                  UserName: config.get('App.webService.userName'),
                  OrgID: '1',
                  ReturnType: 'JSON',
                  Password: config.get('App.webService.password'),
                  AuthType: 'FORM',
                  ProxyID: 0,
                  TokenID: ''
              },
    ...
    var options = {
        "method": "POST",
        "hostname": config.get('App.webService.hostName'),
        "port": null,
        "path": config.get('App.webService.path'),
        "headers": {
...
function GetServiceCatalogDetails(searchString,callback)
{
    ...
    var path = encodeURI(config.get('App.azureSearchAPI.path') + searchString);
    ...
    var options = {
        "method": "GET",
        "hostname": config.get('App.azureSearchAPI.hostName'),
        "port": null,
        "path": path,      
        "headers": {
            "content-type": "application/json",
            "api-key": config.get('App.azureSearchAPI.apiKey'),
            "cache-control": "no-cache"
        }
    };
    var req = http.request(options, function (res) {
